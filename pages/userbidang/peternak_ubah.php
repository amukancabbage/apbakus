...
if(isset($_POST['edit'])){
    ...
    //   $pesanError[]="Kode Kelurahan/Desa sudah ada";
      ...
      if (count($pesanError)>=1 ){  
        ...
        $editQuery	= "UPDATE t_peternak set nama_peternak=?, alamat=?, no_kontak=?, id_desa=?,id_pengguna=? WHERE id=?"; 
...
<form id="demo-form2" data-parsley-validate class="form-horizontal form-label-left" enctype="multipart/form-data" method="POST" >
  <?php 
    input("text","Nama Peternak",1,$data[5],"autofocus required"); 
    input("text","Alamat",2,$data[6],"required"); 
