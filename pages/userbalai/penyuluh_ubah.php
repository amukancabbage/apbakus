...
if(isset($_POST['edit'])){
    ...
    //   $pesanError[]="Kode Kelurahan/Desa sudah ada";
      ...
      if (count($pesanError)>=1 ){  
        ...
        $arrCriteria =array($txt1,$txt2,$txt3,$userid,$txt0);
        $editQuery	= "UPDATE t_penyuluh set nama_penyuluh=?, nip=?, no_kontak=?, id_desa=?,id_pengguna=? WHERE id=?"; 
...
<form id="demo-form2" data-parsley-validate class="form-horizontal form-label-left" enctype="multipart/form-data" method="POST" >
  <?php 
    input("text","Nama Penyuluh",1,$data[5],"autofocus required"); 
    input("text","NIP",2,$data[6],"required"); 
