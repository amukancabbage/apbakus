...
function show_tableJSON($tableName,$isian,$field,$formName,$title,$jmlField){
	$json=file_get_contents("http://localhost/apbakus/api/tipe/read.php");
	...
	foreach ($data->records as $idx => $records) {
		...
		echo("<td align=\"center\">".$nomor++."</td>");
		$i=4;
		echo "<td>$records->tipe</td>";
		echo "<td>$records->deskripsi</td>";
		echo("</td>");
		echo("<td class=\"cc\" align=\"center\"><a href=\"?page=".$nama1."-Ubah&Kode=".encD($Kode)." \" target=\"_self\"><span class=\"btn btn-primary\"><i class=\"fa fa-edit\"></i> Ubah</span></a>");
		echo("<a href=\"?page=Delete&Kode=".encD($Kode)."&table=".encD($tableName)." \" onclick=\"return confirm('Anda Yakin menghapus Data ? ')\"><span class=\"btn btn-danger\"><i class=\"fa fa-trash\"></i> Hapus</span></a></td>");
...
function show_table($koneksidb,$tableName,$isian,$field,$formName,$title,$jmlField,$mySql){
	$nama1 = str_replace("_","-",$formName);
	table_BEGIN($title,4,$jmlField+1,$isian);
	//$mySql = "SELECT $tableName.* FROM ".$tableName;
	$nomor  = 1;
	...
	foreach ($kolomDat as $kolomData) {
		...
		echo("</td>");
		echo("<td class=\"cc\" align=\"center\"><a href=\"?page=".$nama1."-Ubah&Kode=".encD($Kode)." \" target=\"_self\"><span class=\"btn btn-primary\"><i class=\"fa fa-edit\"></i> Ubah</span></a>");
		// echo "<a delete-id='".encD($Kode)."' delete-table='".encD($tableName)."' class='btn btn-danger delete-object'>";
		// echo "<a delete-id='{$Kode}' delete-table='{$tableName}' class='btn btn-danger delete-object'>";
	...
	<div class="x_panel">
		...
		<div class="x_content">
			<table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
				...
				function table_BEGIN($formName,$start,$jmlField,$isian){
					?>
					<div class="x_panel">
						<div class="x_title">
							<h2>Data <?php echo $formName; ?></h2>
